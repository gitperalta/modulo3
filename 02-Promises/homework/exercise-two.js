...
function problemA() {
  ...
  let p1 = promisifiedReadFile("poem-two/stanza-01.txt").then((stanza1) => {
    blue(stanza1);
  });

  let p2 = promisifiedReadFile("poem-two/stanza-02.txt").then((stanza2) => {
    blue(stanza2);
  });
...
function problemB() {
  ...
  let promiseArray = filenames.map((file) =>
  ...
  function promisifiedWriteFile(filename, str) {
    ...
    return new Promise((resolve, reject) => {
      fs.writeFile(filename, str, (err) => {
        if (err) reject(err);
        else resolve("wabaloobadoobdoob");
      });
    });
  }
}
